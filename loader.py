    ...
    def __getitem__(self, index) -> dict:
        ...
        return {
            "input_ids": torch.tensor(
                encoding["input_ids"], dtype=torch.int64
            ).flatten(),
            "attention_mask": torch.tensor(
                encoding["attention_mask"], dtype=torch.int64
            ).flatten(),
            "bbox": torch.tensor(encoding["bbox"], dtype=torch.int64).flatten(
                end_dim=1
            ),
            "pixel_values": torch.tensor(
                encoding["pixel_values"], dtype=torch.float32
            ).flatten(end_dim=1),
